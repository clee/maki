...
- **Env var**: `APERTURE_BASE_URL`
- **API**: `Aperture gateway (set APERTURE_BASE_URL)`
- **Features**: Tailscale Aperture LLM gateway; models discovered from the gateway
...
The `base` field specifies which built-in provider to inherit the model catalog from. Valid values: `anthropic`, `openai`, `google`, `copilot`, `ollama`, `llama-cpp`, `mistral`, `zai`, `deepseek`, `openrouter`, `synthetic`, `tensorx`, `aperture`.
