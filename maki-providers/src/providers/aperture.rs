...
use crate::model::{Model, ModelEntry, ModelInfo, ModelPricing, lookup_entry};
...
struct OverrideFields {
    ...
    /// Override the model's thinking/reasoning support. Wins over the routed
...
fn merged_override(overrides: &Overrides, provider_id: &str, model_id: &str) -> OverrideFields {
    ...
    if let Some(m) = po.models.get(model_id) {
        ...
        if m.supports_thinking.is_some() {
            out.supports_thinking = m.supports_thinking;
        }
    }
    out
}
        ...
        .map(|arr| {
            arr.iter()
                .filter_map(|m| {
                    ...
                    let routed = if provider_id.is_empty() {
                        None
                    } else {
                        routed_kind(provider_id, overrides, id)
                    };
                    let mut info = ModelInfo {
                        ...
                        pricing,
                        supports_thinking: routed.map(kind_supports_thinking),
                    ...
                    if !provider_id.is_empty() {
                        ...
                        if let Some(thinking) = ov.supports_thinking {
                            info.supports_thinking = Some(thinking);
                        }
                    }
                    Some(info)
                })
...
fn apply_adjustments(model: &mut Model, overrides: &Overrides) {
    ...
    if let Some(kind) = routed_kind(provider_id, overrides, model_id) {
        ...
        if let Some(manifest) = manifest_for_kind(kind) {
            if model.supports_thinking_override.is_none() {
                model.supports_thinking_override = Some(manifest.supports_thinking);
            }
            if let Ok(entry) = lookup_entry(manifest.models, model_id) {
                model.context_window = entry.context_window;
                model.max_output_tokens = Some(entry.max_output_tokens);
            }
        }
    }
    ...
    if let Some(mo) = ov.max_output_tokens {
        model.max_output_tokens = Some(mo);
    }
    if let Some(thinking) = ov.supports_thinking {
        model.supports_thinking_override = Some(thinking);
    }
}
...
impl Provider for Aperture {
    ...
    fn adjust_model(&self, model: &mut Model) {
        if let Some((provider_id, model_id)) = model.id.split_once('/') {
            ...
            if let Some(kind) = routed_kind(provider_id, &self.overrides, &model_id) {
...
mod tests {
    ...
    fn apply_adjustments_thinking_follows_routed_kind(spec: &str, kind: ProviderKind) {
        ...
        assert_eq!(
            ...
            Some(kind_supports_thinking(kind))
        );
    ...
    fn adjust_model_inherits_routed_provider_thinking_support() {
        ...
        let aperture = Aperture::with_auth(auth, Timeouts::default());
        let mut model = Model::from_spec("aperture/zai/glm-5.2").unwrap();
        assert!(!model.supports_thinking());
    ...
    fn apply_adjustments_thinking_override_wins_over_route() {
        ...
        let mut model = Model::from_spec("aperture/zai/glm-5.2").unwrap();
        apply_adjustments(&mut model, &overrides);
        assert_eq!(model.supports_thinking_override, Some(true));
        assert!(model.supports_thinking());
    }
    ...
    fn apply_adjustments_thinking_override_disables_routed_capable_model() {
        ...
        apply_adjustments(&mut model, &overrides);
        assert_eq!(model.supports_thinking_override, Some(false));
        assert!(!model.supports_thinking());
    }
